...
# def __cls_info(rel_id, rel, rep):
#     info = rel.split(" ")
#     assert len(info) == 4, f"{rel_id}\t{rel} is not a valid relation"
...
def read_annotation_brat(ann_file, rep=False):
    ...
    with open(ann_file, "r") as f:
        for line in f:
            ...
            if ann_id.startswith("T"):
                ...
                e_cls, entity_id = anns[1].split(':')
                event_id2entity_id_map[ann_id] = entity_id
                # relations.append(__rel_info(ann_id, anns[1], rep))
            elif ann_id.startswith("A"):
                # Debug
                a_cnter += 1
                event_cat, event_id, event_class = anns[1].split(" ")
                a_clses[ann_id] = (event_cat, event_id, event_class)
        # convert A to entities        
        for A_id in a_clses.keys():
            event_cat, event_id, event_class = a_clses[A_id]
            entity_id = event_id2entity_id_map[event_id]
            t_type, entity_words, (offset_s, offset_e) = entites[entity_id2index_map[entity_id]]
            if not events.get(event_cat):
                events[event_cat] = []
            events[event_cat].append((t_type, event_class, (offset_s, offset_e)))
    ...
    return entity_id2index_map, entites, events, a_cnter
