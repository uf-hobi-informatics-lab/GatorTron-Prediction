...
###########new3
    def get_test_examples_highlight(self, input):
        """See base class."""

        return self._create_examples_highlight(input, "test")
###########new3
    ...
    def get_labels(self, train_file=None, label_file=None):
        ...
        try:
            # trans to int
            unique_labels.sort(key=lambda x: int(x))
...
###############new4
    def _create_examples_helper_highlight(self, lines_idx, set_type, total_special_toks):
        start_idx, input = lines_idx
        examples = []
        guid = "{}_{}_0".format(set_type, start_idx)
        text_a = input
        # text_b = line[2]
        label = "0" #set random in string type
    ...
    def _create_examples_highlight(self, input, set_type='test'):
        """Creates examples for the training and dev sets."""
        ...
        if self.num_core < 2:
            # single process - maybe too slow - replace with multiprocess
            examples = self._create_examples_helper_highlight((0, input), set_type, self.total_special_token_num)
        else:
            # multi-process - assume the first read-in data is the csv title with no data information
            # use multi-cores to process data if you have many long sentences;
            # otherwise single process should be faster
            examples = []
            array_lines = np.array_split(input, self.num_core)
            with ProcessPoolExecutor(max_workers=self.num_core) as exe:
                for each in exe.map(partial(self._create_examples_helper_highlight,
                                            set_type=set_type,
                                            total_special_toks=self.total_special_token_num),
                                    enumerate(array_lines)):
                    examples.extend(each)
...
###############new4
    ...
    def _process_seq_len(self, text_a, total_special_toks=3):
        ...
        """
        flag = True
        #print('token_count_texta',len(self.tokenizer.tokenize(text_a)),flush=True)
        #print(len(self.tokenizer.tokenize(text_a)), flush=True)
        while len(self.tokenizer.tokenize(text_a)) \
                > (self.max_seq_len - total_special_toks):
            #print('kang', flush=True)
