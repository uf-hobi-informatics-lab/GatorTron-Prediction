...
def app(gargs):
  ...
  ############new        
    if gargs.do_predict_highlight:
        # run prediction highlighr
        exp = task_runner.predict_highlight(gargs.highlight_index)
        # exp.save_to_file('/orange/yonghui.wu/chenziyi/Highlight/LIME/HF/lung60.html',text=True)
        exp.save_to_file(gargs.highlight_output_file,text=True)
    ############new      
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Whether to run prediction highlight on the test set. (require test.tsv)",
    )

    parser.add_argument(
        "--highlight_output_file",
        type=str,
        default=None,
        help="highlight results output file.",
    )
    
    parser.add_argument("--highlight_index", default=0, type=int, help="highlight index")
